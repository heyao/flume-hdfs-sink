...
import org.apache.flume.sink.hdfs.HDFSEventSink.PathInfo;
...
class BucketWriter {
  ...
  //ray
  public void registerBucketWriter() {
	  String p=this.filePath;
		if (!"-".equals(this.doneFlag)) {
			 Map<String,PathInfo> m_path=HDFSEventSink.m_path;
			synchronized (m_path) {
				if(m_path.containsKey(p)){
					m_path.get(p).bucketWriterCounter++;
				}else{
					m_path.put(p, new PathInfo(1,headers));
				}
				LOG.info("---------[++]["+m_path.get(p).bucketWriterCounter+"]"+p);
			}
		}
  }

  //ray
public void unregisterBucketWriter() throws IOException{
		if (!"-".equals(this.doneFlag)) {
			 Map<String,PathInfo> m_path=HDFSEventSink.m_path;
			String p = filePath;
			synchronized (m_path) {
				if (m_path.containsKey(p)) {
					...
					throw new RuntimeException("no path[" + p + "] in m_path");
				}
				LOG.info("---------[--][" + m_path.get(p).bucketWriterCounter + "]" + p);
			}
		}
